...
delay(10); //wait 10ms between pings:
 unsigned int distance = sonar.ping_cm(); //send ping, get distance in cm and store it in 'distance' variable:
Serial.print("distance");
Serial.println(distance); // print the distance in serial monitor:
int Right_Value = digitalRead(RIGHT); // read the value from Right IR sensor:
int Left_Value = digitalRead(LEFT); // read the value from Left IR sensor:
Serial.print("RIGHT");
...
if((Right_Value==1) && (distance>=10 && distance<=30) && (Left_Value==1)){ //check wheather the ultrasonic sensor's value stays between 1 to 15.
//Move Forward:
Motor1.setSpeed(350); //define motor1 speed:
Motor1.run(FORWARD); //rotate motor1 clockwise:
Motor2.setSpeed(350); //define motor2 speed:
Motor2.run(FORWARD); //rotate motor2 clockwise:
Motor3.setSpeed(350); //define motor3 speed:
Motor3.run(FORWARD); //rotate motor3 clockwise:
Motor4.setSpeed(350); //define motor4 speed:
...
Motor4.run(FORWARD); //rotate motor4 clockwise:
delay(50);
}else if(distance > 1 && distance<10) { //If the condition is 'true' then the statement below will execute:
//Stop
